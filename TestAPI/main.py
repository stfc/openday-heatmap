...
app.add_middleware(DBSessionMiddleware, db_url="postgresql://postgres:root@localhost:5432/postgres")
...
async def Tracking():
    db_Tracking = ModelTracking(TrackingID= str(uuid.uuid4()), UserID=str( random.randint(1, 100)) , QRID=str(random.randint(1, 17)))
    db.session.add(db_Tracking)
    db.session.commit()
...
async def Tracking():
    one_day_interval_before = datetime.now() - timedelta(seconds=1) - timedelta(hours=1)
